...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, CreateClaimModalComponent, AuthenticatedNavbarComponent, LocalizedFieldPipe],
...
export class ExternalClaimsListComponent implements OnInit {
  ...
  loadClaims(): void {
    ...
    this.claimService.getClaimsByUserFiltered(params).subscribe({
      next: (response) => {
