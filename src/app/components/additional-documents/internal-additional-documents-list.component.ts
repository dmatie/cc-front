...
@Component({
  ...
  templateUrl: './internal-additional-documents-list.component.html',
  styleUrls: ['./internal-additional-documents-list.component.css']
})
export class InternalAdditionalDocumentsListComponent implements OnInit {
  ...
  loadDocuments(): void {
    ...
    const query: GetOtherDocumentsWithFiltersQuery = {
    ...
    this.otherDocumentService.getOtherDocumentsWithFilters(query).subscribe({
      next: (response) => {
  ...
  getUserFullName(doc: OtherDocumentDto): string {
    if (doc.user) {
